...
import Impl.QueryVideoDanmakuMessagePlanner
import Impl.DeleteDanmakuMessagePlanner
import Impl.PublishDanmakuMessagePlanner
import Impl.QueryDanmakuByIDMessagePlanner
...
object Routes:
  ...
  private def executePlan(messageType: String, str: String): IO[String] =
    messageType match {
      case "QueryVideoDanmakuMessage" =>
        IO(
          decode[QueryVideoDanmakuMessagePlanner](str) match
            case Left(err) => err.printStackTrace(); throw new Exception(s"Invalid JSON for QueryVideoDanmakuMessage[${err.getMessage}]")
            case Right(value) => value.fullPlan.map(_.asJson.toString)
        ).flatten
       
      case "DeleteDanmakuMessage" =>
        IO(
          decode[DeleteDanmakuMessagePlanner](str) match
            case Left(err) => err.printStackTrace(); throw new Exception(s"Invalid JSON for DeleteDanmakuMessage[${err.getMessage}]")
            case Right(value) => value.fullPlan.map(_.asJson.toString)
        ).flatten
       
      case "PublishDanmakuMessage" =>
        IO(
          decode[PublishDanmakuMessagePlanner](str) match
            case Left(err) => err.printStackTrace(); throw new Exception(s"Invalid JSON for PublishDanmakuMessage[${err.getMessage}]")
            case Right(value) => value.fullPlan.map(_.asJson.toString)
        ).flatten
       
      case "QueryDanmakuByIDMessage" =>
        IO(
          decode[QueryDanmakuByIDMessagePlanner](str) match
            case Left(err) => err.printStackTrace(); throw new Exception(s"Invalid JSON for QueryDanmakuByIDMessage[${err.getMessage}]")
            case Right(value) => value.fullPlan.map(_.asJson.toString)
        ).flatten
