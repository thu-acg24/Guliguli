...
case class EndTransactionMessagePlanner(commit: Boolean) extends DBPlanner[String] {
  override def planWithConnection(connection: Connection)(using planContext:PlanContext): IO[String] = {
    IO.delay(if (commit) {
      ...
      result=>
        ...
        IO(connection.close())>>
        IO(connectionMap.update(_ - planContext.traceID.id)) >>
        IO(result)
    )
  }
}