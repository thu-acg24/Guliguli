...
abstract class API[T: Decoder](targetService: String):
  ...
  @JsonIgnore def getURIWithAPIMessageName: IO[Uri] =
    for {
  ...
  @JsonIgnore def send(using Encoder[this.type], PlanContext): IO[T] = API.send[T, this.type](this)
...
object API {
  ...
  def init(maximumClientConnection: Int): IO[Unit] = {
    val clientResource: Resource[IO, Client[IO]] = EmberClientBuilder.default[IO]
      ...
      .withIdleConnectionTime(900.seconds)
      .build
  ...
  def getClient(): Client[IO] = client.getOrElse(throw new Exception("HTTP client is not initialized"))
